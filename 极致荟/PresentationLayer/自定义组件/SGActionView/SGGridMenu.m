...
- (void)layoutSubviews
{
    ...
    self.contentScrollView.frame = (CGRect){CGPointMake(0,0), self.contentScrollView.bounds.size};
    
    self.cancelButton.frame = CGRectMake(self.bounds.size.width*0.05,self.contentScrollView.bounds.size.height, self.bounds.size.width*0.9, 44);
...
- (void)layoutContentScrollView
{
    ...
    CGSize itemSize = CGSizeMake((self.bounds.size.width - margin.left - margin.right) / 3, (self.bounds.size.width - margin.left - margin.right - self.cancelButton.bounds.size.height) / 3);
